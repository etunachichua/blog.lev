...
class users_model extends CI_Model {
    ...
    public function getSalt($username) {
        $this->db->where('personalID', $username);
    ...
    //momxmareblis shemocmeba 
    public function CheckLogin($personalID, $password) {
    ...
    public function getProfilePersonalID($userId) {
        $this->db->where('personalID', $userId);
        return $this->db->get('users')->row();
    }
    ...
    public function passChange($userId) {
        //Password generation
        $this->oldpass = $this->getPassword($this->session->userdata('personalID'), $this->input->post('currentPassword'));
    ...
    //parolis shecvla
    public function passremember($userId,$password) {
        $this->user = $this->getProfilePersonalID($userId);
        $this->smscode = $password;
        ...
        $this->password = $this->setPassword($this->password,$this->salt);
                ...
                $query = $this->db->update('users', $data);
                if ($query) {
                    $smstext = 'tqveni axali paroli sistemashi shesasvlelad:'.$this->smscode;
                    $this->customsmssend($this->user->id, '995'.$this->user->phone, $this->smscode,$smstext);
                    $message = '<div class="alert alert-success fade in"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>პაროლი წარმატებით შეიცვალა</div>';
                } else {
                    $message = '<div class="alert alert-warning fade in"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>შეცდომა: პაროლი არ შეიცვალა!</div>';
                }
